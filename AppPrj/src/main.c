...
void main()
{
    ...
    while(1)
    {
    	...
    	if(length >= 5 || (RS422_STA_REG(10) >> 4) == 3)
    	{
    		rs422Trans(10,pbuf,5);
    		if(pbuf[0] == 0xaa && pbuf[1] == 0x55)
    		{
    			if(checksum(pbuf) == pbuf[4])
    			{
    				channel = pbuf[3]-1;
    				...
    				{
        				rs422Write(channel,test,100);
        				while(timeout < 0xfffff)
        				{
        					length = RS422_RX_SIZE_REG(channel);
        					if(length >= 100 || (RS422_STA_REG(channel) >> 4 == 3))
        					{
        						rs422Trans(channel,cmp,100);
