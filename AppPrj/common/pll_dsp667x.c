/*
 閺傚洣娆㈤崥宥囆為敍姝眑l_dsp667x.c
閹芥顪呴敍姘瘜鐟曚礁鐤勯悳鐨僱l_dsp667x閻ㄥ嚤LL閻ㄥ嫬鍨垫慨瀣
*/
/* 娣囶喗鏁奸崢鍡楀蕉:
2016.3.11,chengqb閸掓稑缂撻弬鍥︽
*/
...
LOCAL INT32 pll0Init()	/*main pll*/
{
	...
	#if 0
	if(pCfg->pllParamtertFlag != PARAMTER_FLAG_MASK) //娴ｈ法鏁ゆ妯款吇閸欏倹鏆�
	{
...
LOCAL CSL_Status corePllcHwSetup (PllcHwSetup *hwSetup)
{
	...
	/* Wait for at least 500 鑴�CLKIN cycles 鑴�(PLLD + 1)   */
	for (i = 0; i < 100; )
	{
		pllDelay(300);
	...
	#if 1//alj2014_4_10 PLL閸婂秹顣舵稉宥嗗灇閸旓拷
...
unsigned int getcore_clk ()
{
 ...
 LOCAL INT32 corePllcHwSetup(PllcHwSetup *hwSetup)
 {
     ...
     if(temp != 0) /* PLL BYPASS is enabled, we assume if not in Bypass ENSAT = 1 */
     {
         ...
         pllDelay(14005);            //Wait for at least 5 濞撶捈 based on the reference clock CLKIN
     ...
     pllDelay(14006);                //Wait for at least 7 濞撶捈 based on the reference clock CLKIN
     PLLCTL_REG &= ~(0x00000008);    //PLL reset is de-asserted
 
     pllDelay(140056 >> 1);          //Wait for at least 500 閼达拷CLKIN cycles 閼达拷(PLLD + 1) (PLL lock time)
